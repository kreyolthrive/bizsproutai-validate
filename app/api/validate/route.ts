...
function parseValidationPayload(
  ...
  const input: ValidationInput = {
    ...
    experienceLevel: asTrimmedString(raw.experienceLevel),
  };
...
function buildFallbackReport(args: {
  ...
  const text = [
    ...
    "Next Steps",
    "----------",
    ...(nextSteps.length ? nextSteps : ["No next steps returned."]).map((item) => `- ${item}`),
    "",
...
async function persistValidationRun(args: {
  ...
  try {
    ...
    const saveBusinessValidationRun = (mod as Record<string, unknown>).saveBusinessValidationRun;
...
async function deliverEmails(args: {
  ...
  try {
    ...
    const sendValidationEmails = (mod as Record<string, unknown>).sendValidationEmails;

    if (typeof sendValidationEmails !== "function") {
      throw new Error("sendValidationEmails export was not found.");
    }

    const emailResult = await (sendValidationEmails as (payload: unknown) => Promise<unknown>)({
      requestId: args.requestId,
      email: args.email,
      to: args.email,
      locale: args.input.locale ?? "en",
